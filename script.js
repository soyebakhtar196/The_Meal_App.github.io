...
async function showMealList() {
    ...
    let cardForMeal = '';
    if (mealsData.meals) {
        cardForMeal = mealsData.meals.map(element => {
        ...
        document.getElementById('cards-holder').innerHTML = cardForMeal;
    }
}
...
async function showMealDetails(itemId, searchInput) {
    ...
    if( mealList.meals!=null){
        detailsPage += mealList.meals.map(element => {
            return `       
            <div class="card">
    ...
    detailsPage = detailsPage + '</div>';

    document.getElementById('flex-box').innerHTML = detailsPage;
}
...
async function showFavMealList() {
    ...
    if (favList.length == 0) {
        favListPage = `<div class="fav-item nothing"> <h1> 
    ...
    document.getElementById('fav').innerHTML = favListPage;
}
